...
$d_z,\;d_s$ & Model-dependent, $d_s=2048$ & Semantic latent dimension and smashed-feature size. \\
...
$\omega_1,\omega_2,\omega_3,\tau,\eta$ & BCBSA internal coefficients & Trust-score weights, acceptance threshold, and temporal EMA rate. \\
...
\begin{itemize}
\item Conv2D layer: $3 \rightarrow 64$, kernel $7\times7$, stride 2, padding 3
\item BatchNorm + ReLU + MaxPool2d ($3\times3$, stride 2)
...
\begin{equation}
\mathbf{s}_i \in \mathbb{R}^{128\times4\times4}.
\end{equation}
The smashed feature size is therefore
\begin{equation}
d_s = 128 \times 4 \times 4 = 2048.
\end{equation}
The corresponding per-sample payload before compression is
\begin{equation}
B_{\mathrm{raw}} = 32\,d_s = 65{,}536\;\text{bits} = 8\;\text{KB}.
\end{equation}
...
\label{alg:bcbsa}
\begin{algorithmic}[1]
\REQUIRE Gradient-statistic tuples $\mathcal{G}=\{(g_k,F_k,D_k)\}_{k=1}^{K}$, weights $(\omega_1,\omega_2,\omega_3)$, acceptance threshold $\tau$, EMA rate $\eta$, reference gradient $\hat{g}^{t-1}$ (EMA of previous accepted mean)
\ENSURE Aggregated gradient $\hat{g}$
\FOR{each client $k=1,\ldots,K$}
\STATE Compute normalised distortion $\bar{D}_k \leftarrow \min\!\left(1,\,\frac{\|\tilde{z}_k-z_k\|^2}{d_z}\right)$
\STATE Compute raw semantic trust score\[\tau_k^{\mathrm{raw}} \leftarrow \omega_1\max(0,F_k) + \omega_2(1-\bar{D}_k) + \omega_3\max\!\left(0,\cos(g_k,\hat{g}^{t-1})\right)\]
\STATE Apply temporal EMA\[\tau_k \leftarrow (1-\eta)\tau_k^{t-1} + \eta\,\tau_k^{\mathrm{raw}}\]
\ENDFOR
\STATE Build accepted set $\mathcal{A} \leftarrow \{k : \tau_k > \tau\}$; if $\mathcal{A}=\varnothing$ set $\mathcal{A}\leftarrow\{1,\ldots,K\}$
\STATE Compute softmax weights $w_k \leftarrow \exp(\tau_k)/\sum_{j\in\mathcal{A}}\exp(\tau_j)$ for $k\in\mathcal{A}$
\STATE Compute weighted aggregate\[\hat{g} \leftarrow \sum_{k\in\mathcal{A}} w_k\, g_k\]
\STATE Update reference gradient $\hat{g}^t \leftarrow \hat{g}$
\RETURN $\hat{g}$
\end{algorithmic}
\end{algorithm}
